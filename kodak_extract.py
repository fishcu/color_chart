...
INVALID_CELLS_X_BEG = 19
INVALID_CELLS_X_END = 21
INVALID_CELLS_Y_BEG = 0
INVALID_CELLS_Y_END = 7
...
class KodakExtractor:
    def __init__(self, points):
        ...
        self.num_cells_x = NUM_CELLS_X  # Number of cells in x direction
        self.num_cells_y = NUM_CELLS_Y  # Number of cells in y direction

        # Calculate the width and height of the undistorted image by summing the widths and heights of the distorted quadrilateral
        width_A_B = np.linalg.norm(
            np.array(self.points[0]) - np.array(self.points[1]))
        width_C_D = np.linalg.norm(
            np.array(self.points[2]) - np.array(self.points[3]))
        height_A_D = np.linalg.norm(
            np.array(self.points[0]) - np.array(self.points[3]))
        height_B_C = np.linalg.norm(
            np.array(self.points[1]) - np.array(self.points[2]))
        ...
        dst_points = np.float32([[0, 0], [self.out_width - 1, 0],
                                [self.out_width - 1, self.out_height - 1], [0, self.out_height - 1]])
    ...
    def calculate_perspective_transform(self, img):
        ...
        undistorted_img = cv2.warpPerspective(
            img, self.M, (self.out_width, self.out_height))
    ...
    def divide_into_cells_and_draw_rectangles(self, undistorted_img):
        ...
        num_valid_cells = self.num_cells_x * self.num_cells_y - \
            (INVALID_CELLS_X_END - INVALID_CELLS_X_BEG + 1) * \
            (INVALID_CELLS_Y_END - INVALID_CELLS_Y_BEG + 1)
        median_colors = np.zeros((num_valid_cells, 3))
        num_proc = 0

        for i in range(self.num_cells_x):
            for j in range(self.num_cells_y):
                # Skip the invalid block
                if INVALID_CELLS_X_BEG <= i <= INVALID_CELLS_X_END and INVALID_CELLS_Y_BEG <= j <= INVALID_CELLS_Y_END:
                    continue

                top_left = (int(i * cell_width), int(j * cell_height))
                bottom_right = (int((i + 1) * cell_width),
                                int((j + 1) * cell_height))
                ...
                # rect_top_left = (int(top_left[0] + (cell_width - rect_width) / 2), int(
                #     top_left[1] + (cell_height - rect_height) / 2))
                # rect_bottom_right = (
                #     int(rect_top_left[0] + rect_width), int(rect_top_left[1] + rect_height))

                # Draw the rectangle on the image
                # cv2.rectangle(undistorted_img, rect_top_left,
                #               rect_bottom_right, (0, 255, 0), 3)

                # Extract the cell region
                cell_img = undistorted_img[top_left[1]
    ...
    def get_rgb_values(self, img):
        ...
        result_img, median_colors = self.divide_into_cells_and_draw_rectangles(
            undistorted_img)
...
def draw_cell_grid(cell_colors, num_cells_x, num_cells_y, cell_size, skip_column=1, skip_color=[128, 128, 128]):
    ...
    for i in range(num_cells_x):
        for j in range(num_cells_y):
            ...
            # Fill the cell with the specified color or the skip color
            if INVALID_CELLS_X_BEG <= i <= INVALID_CELLS_X_END and INVALID_CELLS_Y_BEG <= j <= INVALID_CELLS_Y_END:
                color = skip_color
            else:
            ...
            color[0] = color[2]
            color[2] = tmp
            cv2.rectangle(grid_image, (top_left_x, top_left_y),
                          (top_left_x + cell_size, top_left_y + cell_size), color, -1)

    # Draw the grid lines
    for i in range(1, num_cells_x):
        cv2.line(grid_image, (i * cell_size, 0),
                 (i * cell_size, img_height), (255, 255, 255), 1)
    for j in range(1, num_cells_y):
        cv2.line(grid_image, (0, j * cell_size),
                 (img_width, j * cell_size), (255, 255, 255), 1)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        grid_image = draw_cell_grid(
            median_colors, NUM_CELLS_X, NUM_CELLS_Y, 50)
